...
function eachMonthTestSuit(CURRENT) {
  describe(CURRENT.format("MMMM") + " case", () => {
    const FIRST_DATE = CURRENT.clone().startOf("months");
    const LAST_DATE = CURRENT.clone().endOf("month");
    ...
    it("sundays to staurdays", (done) => {
      const DAYS_WEEK_ARR = getDaysWeekArray(CURRENT);
      const NUM_DAYS = CURRENT.endOf("month").date();
      let counter = 0;
    ...
    it("4 weeks dates and last week first day", (done) => {
      let DAYS_WEEK_ARR = getDaysWeekArray(CURRENT);
...
describe("Helper", () => {
  describe("daysWeekArray", () => {
    const TODAY = moment();
    const ND_MONTH = TODAY.clone().add(1, 'months');
    const RD_MONTH = TODAY.clone().add(2, 'months');

    eachMonthTestSuit(TODAY);

    eachMonthTestSuit(ND_MONTH);

    eachMonthTestSuit(RD_MONTH);

    it("input date", (done) => {
      const DAYS_WEEK_ARR = getDaysWeekArray(new Date);
      const NUM_DAYS = TODAY.clone().endOf("month").date();
      let counter = 0;
